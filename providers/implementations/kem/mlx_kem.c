...
static int hybrid_concat_hkdf(
    const unsigned char *kem,
    size_t kem_len,
    const unsigned char *ecdh,
    size_t ecdh_len,
    unsigned char *out,
    size_t out_len,
    OSSL_LIB_CTX *libctx
)
{
    ...
    memcpy(secret, kem, kem_len);
    memcpy(secret + kem_len, ecdh, ecdh_len);

    secret_len = kem_len + ecdh_len;

    kdf = EVP_KDF_fetch(libctx, "HKDF", NULL);
    if (kdf == NULL)
        return 0;
    ...
    unsigned char *shsec, size_t *slen)
{
	...
	{
    	unsigned char hybrid_secret[32];

    	unsigned char *kem_secret =
    		shsec + ml_kem_slot
    		      * ML_KEM_SHARED_SECRET_BYTES;

    	unsigned char *ecdh_secret =
    	    shsec + (1 - ml_kem_slot)
    	          * ML_KEM_SHARED_SECRET_BYTES;

    	if (!hybrid_concat_hkdf(
    	        kem_secret,
    	        ML_KEM_SHARED_SECRET_BYTES,
    	        ecdh_secret,
    	        key->xinfo->shsec_bytes,
    	        hybrid_secret,
